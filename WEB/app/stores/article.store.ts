...
import { GET_ARTICLE_BY_ID_GROQ, GET_FOUR_LAST_ARTICLE_GROQ, GET_LAST_ARTICLE_GROQ } from "~/utils/groq/article.groq"
...
export const useArticleStore = defineStore("article-store", () => {
    ...
    const GetArticleById = async (id: string) => {
        loader.Show()
        try {
            const result = await SANITY_CLIENT.fetch(GET_ARTICLE_BY_ID_GROQ, { id })
            loader.Hide()
            return result

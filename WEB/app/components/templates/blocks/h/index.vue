    ...
    <PageLayout custom-class="bg-primary py-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 py-10">
            <div v-for="(item, index) in (props.data?.items as BaseType[])" :key="index"
                class="card group transition-all duration-300 border-1 border-tertiary/50 bg-tertiary/10 p-4 rounded-lg flex flex-col justify-between">
                <div class="text-center py-4">
                    ...
                    <h3 class="text-tertiary text-lg font-semibold mb-3 min-h-auto md:min-h-15">
                        {{ item?.name?.[locale] }}
                    </h3>

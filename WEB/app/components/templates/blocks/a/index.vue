<template>
    <PageLayout custom-class="bg-primary py-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 py-10">
            <div v-for="(item, index) in (props.data?.items as BaseType[])" :key="index"
                class="card group transition-all duration-300 border-1 border-tertiary/50 bg-tertiary/10 p-4 rounded-lg">
                <div class="text-center py-4">
                    <div class="w-16 h-16  rounded-lg flex items-center justify-center mx-auto mb-4 transition-fast">
                        <Icon :name="item?.icon ?? ''" class="h-16 w-16 text-accent-200 transition-all duration-400" />
                    </div>
                    <h3 class="text-tertiary text-lg font-semibold mb-3 min-h-auto md:min-h-20">
                        {{ item?.name?.[locale] }}
                    </h3>
                    <p class="mb-4 text-center text-md text-tertiary-500/80">
                        {{ item?.text?.[locale] }}
                    </p>
                    <div class="opacity-0 transition-fast w-full flex justify-center">
